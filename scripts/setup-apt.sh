...
# Usage: curl -sSL https://raw.githubusercontent.com/CDUESTC-OpenAtom-Open-Source-Club/OpenTenBase-Packages/main/scripts/setup-apt.sh | sudo bash
#
# Options:
...
fetch_from_mirror() {
    ...
    # Fallback to GitHub
    local github_url="https://cduestc-openatom-open-source-club.github.io/OpenTenBase-Packages/apt${path}"
...
# main = v5.0 (latest), v2.6 = v2.6.0, v2.5 = v2.5.0
case "$REQUESTED_VERSION" in
    2.6.0|2.6) APT_COMPONENT="v2.6" ;;
    2.5.0|2.5) APT_COMPONENT="v2.5" ;;
    5.0|"")    APT_COMPONENT="main" ;;
    *)         log_error "Unsupported version: $REQUESTED_VERSION"; echo "Supported: 5.0, 2.6.0, 2.5.0"; exit 1 ;;
esac

detect_mirror
REPO_URL="https://github.com/CDUESTC-OpenAtom-Open-Source-Club/OpenTenBase-Packages/releases/latest/download"
...
add_gpg_key() {
    ...
    for url in "$GPG_KEY_URL" "https://cduestc-openatom-open-source-club.github.io/OpenTenBase-Packages/apt/gpg-key.asc"; do
...
deb [signed-by=$KEYRING_PATH] ${APT_REPO_URL} $CODENAME $APT_COMPONENT
EOF
...
show_install_info() {
    ...
    echo "========================================"
    echo ""
    echo "  配置的版本: ${REQUESTED_VERSION:-5.0} (component: $APT_COMPONENT)"
    echo ""
    echo "安装 OpenTenBase:"
    echo ""
    ...
    echo ""
    echo "安装其他版本 (修改 sources.list 中的 component):"
    echo "  main  = v5.0 (最新)"
    echo "  v2.6  = v2.6.0"
    echo "  v2.5  = v2.5.0"
    echo ""
