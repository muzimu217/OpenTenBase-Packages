#!/bin/bash
# OpenTenBase Docker entrypoint (dev version)
# Uses binaries from shared volume at /usr/lib/opentenbase/5.0/
...
query_coordinator() {
    $OTB_BIN/psql -h "$COORD_HOST" -p "$COORD_PORT" -U opentenbase -d postgres -t -A -c "$1" 2>/dev/null || true
}
...
# Resolve hostnames to IPs at startup
...
gtm)
    if [ ! -f "$DATA_DIR/gtm.conf" ]; then
        ...
        $OTB_BIN/initgtm -Z gtm -D "$DATA_DIR"
...
EOF
    ...
    exec $OTB_BIN/gtm -D "$DATA_DIR" -l /var/lib/opentenbase/data/gtm.log
    ;;
...
coordinator)
    if [ ! -f "$DATA_DIR/postgresql.conf" ]; then
        ...
        $OTB_BIN/initdb -D "$DATA_DIR" --nodename=coordinator --nodetype=coordinator \
...
EOF
    ...
    $OTB_BIN/postgres --coordinator -D "$DATA_DIR" &
    COORD_PID=$!

    wait_for_port "127.0.0.1" "$COORD_PORT" 30

    log "Registering nodes..."
    $OTB_BIN/psql -h 127.0.0.1 -p "$COORD_PORT" -U opentenbase -d postgres -c \
    ...
    # Resolve datanode hostnames to IPs
...
datanode)
    if [ ! -f "$DATA_DIR/postgresql.conf" ]; then
        ...
        $OTB_BIN/initdb -D "$DATA_DIR" --nodename="$NODE_NAME" --nodetype=datanode \
            --master_gtm_nodename=gtm_master --master_gtm_ip="$GTM_HOST" --master_gtm_port="$GTM_PORT"
        cat >> "$DATA_DIR/postgresql.conf" <<EOF
port = $DN_PORT
listen_addresses = '*'
...
EOF
    ...
    $OTB_BIN/postgres --datanode -D "$DATA_DIR" &
    DN_PID=$!

    wait_for_port "127.0.0.1" "$DN_PORT" 30

    # Synchronize node_ids with coordinator
    ...
    if [ -z "$COORD_NODE_ID" ] || [ -z "$MY_NODE_ID" ]; then
        log "ERROR: Could not read node_ids from coordinator"
        exit 1
    fi
    ...
    # Fix pgxc_node on this datanode
...
EOSQL
    ...
    $OTB_BIN/psql -h 127.0.0.1 -p "$DN_PORT" -U opentenbase -d postgres -f "$SQL_FILE" 2>&1 || true
    rm -f "$SQL_FILE"
    $OTB_BIN/psql -h 127.0.0.1 -p "$DN_PORT" -U opentenbase -d postgres -c \
        "SELECT pgxc_pool_reload();" 2>/dev/null || true

    # If datanode1, create node group after all datanodes are ready
    if [ "$NODE_NAME" = "datanode1" ]; then
        ...
        for i in $(seq 1 5); do
            RESULT=$($OTB_BIN/psql -h "$COORD_HOST" -p "$COORD_PORT" -U opentenbase -d postgres -t -A -c \
        ...
        $OTB_BIN/psql -h "$COORD_HOST" -p "$COORD_PORT" -U opentenbase -d postgres -c \
            "SELECT pgxc_pool_reload();" 2>/dev/null || true
