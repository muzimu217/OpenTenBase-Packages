...
query_coordinator() {
    /usr/lib/opentenbase/bin/psql -h "$COORD_HOST" -p "$COORD_PORT" -U opentenbase -d postgres -t -A -c "$1" 2>/dev/null || true
}
...
gtm)
    if [ ! -f "$DATA_DIR/gtm.conf" ]; then
        ...
        /usr/lib/opentenbase/bin/initgtm -Z gtm -D "$DATA_DIR"
...
EOF
    ...
    exec /usr/lib/opentenbase/bin/gtm -D "$DATA_DIR" -l /var/lib/opentenbase/data/gtm.log
    ;;
...
coordinator)
    if [ ! -f "$DATA_DIR/postgresql.conf" ]; then
        ...
        /usr/lib/opentenbase/bin/initdb -D "$DATA_DIR" --nodename=coordinator --nodetype=coordinator \
...
EOF
    ...
    /usr/lib/opentenbase/bin/postgres --coordinator -D "$DATA_DIR" &
    COORD_PID=$!

    wait_for_port "127.0.0.1" "$COORD_PORT" 30

    log "Registering nodes in pgxc_node..."
    /usr/lib/opentenbase/bin/psql -h 127.0.0.1 -p "$COORD_PORT" -U opentenbase -d postgres -c \
...
datanode)
    if [ ! -f "$DATA_DIR/postgresql.conf" ]; then
        ...
        /usr/lib/opentenbase/bin/initdb -D "$DATA_DIR" --nodename="$NODE_NAME" --nodetype=datanode \
            --master_gtm_nodename=gtm_master --master_gtm_ip="$GTM_HOST" --master_gtm_port="$GTM_PORT"
        cat >> "$DATA_DIR/postgresql.conf" <<EOF
port = $DN_PORT
listen_addresses = '*'
...
EOF
    ...
    wait_for_port "$GTM_HOST" "$GTM_PORT"
    wait_for_port "$COORD_HOST" "$COORD_PORT"

    log "Starting Datanode on port $DN_PORT..."
    /usr/lib/opentenbase/bin/postgres --datanode -D "$DATA_DIR" &
    DN_PID=$!
...
EOSQL
    ...
    /usr/lib/opentenbase/bin/psql -h 127.0.0.1 -p "$DN_PORT" -U opentenbase -d postgres -f "$SQL_FILE" 2>&1 || true
    rm -f "$SQL_FILE"
    /usr/lib/opentenbase/bin/psql -h 127.0.0.1 -p "$DN_PORT" -U opentenbase -d postgres -c \
        "SELECT pgxc_pool_reload();" 2>/dev/null || true
    ...
    if [ "$NODE_NAME" = "datanode1" ]; then
        ...
        for i in $(seq 1 5); do
            RESULT=$(/usr/lib/opentenbase/bin/psql -h "$COORD_HOST" -p "$COORD_PORT" -U opentenbase -d postgres -t -A -c \
        ...
        /usr/lib/opentenbase/bin/psql -h "$COORD_HOST" -p "$COORD_PORT" -U opentenbase -d postgres -c \
            "SELECT pgxc_pool_reload();" 2>/dev/null || true
